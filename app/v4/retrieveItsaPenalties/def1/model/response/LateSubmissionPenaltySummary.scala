...
case class LateSubmissionPenaltySummary(
    ...
    periodOfComplianceAchievement: Option[String],
    regimeThreshold: Int,
...
object LateSubmissionPenaltySummary {
  ...
  implicit val reads: Reads[LateSubmissionPenaltySummary] = (
    (JsPath \ "activePenaltyPoints").read[Int] and
      (JsPath \ "inactivePenaltyPoints").read[Int] and
      (JsPath \ "pocAchievementDate").readNullable[String] and
      (JsPath \ "regimeThreshold").read[Int] and
      (JsPath \ "penaltyChargeAmount").read[BigDecimal]
  )(LateSubmissionPenaltySummary.apply)

  implicit val writes: OWrites[LateSubmissionPenaltySummary] =
    Json.writes[LateSubmissionPenaltySummary]
