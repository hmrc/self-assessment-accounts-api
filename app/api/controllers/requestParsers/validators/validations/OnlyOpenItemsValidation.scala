...
import api.models.errors.{MtdError, RuleInconsistentQueryParamsError}
...
object OnlyOpenItemsValidation {
  ...
  def validate(onlyOpenItems: Option[String], docNumber: Option[String], toDate: Option[String], fromDate: Option[String]): List[MtdError] = {
    ...
    if (onlyOpenItems.contains("true") && otherQueryParamsDefined) {
      List(RuleInconsistentQueryParamsError)
    } else {
      Nil
    }
  }
