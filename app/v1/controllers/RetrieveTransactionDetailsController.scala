...
class RetrieveTransactionDetailsController @Inject()(val authService: EnrolmentsAuthService,
  ...
  def retrieveTransactionDetails(nino: String, transactionId: String): Action[AnyContent] = authorisedAction(nino).async{
    implicit request =>
      val rawRequest: RetrieveTransactionDetailsRawRequest = RetrieveTransactionDetailsRawRequest (nino, transactionId)
      val result =
        for {
...
//            hateoasFactory
//              .wrapList(serviceResponse.responseData, RetrieveTransactionDetailsHateoasData(nino))
  ...
  private def errorResult(errorWrapper: ErrorWrapper) = {
    (errorWrapper.error: @unchecked) match {
      case BadRequestError | NinoFormatError
           | TransactionIdFormatError => BadRequest(Json.toJson(errorWrapper))
