...
import v4.retrieveItsaPenalties.model.response.RetrieveItsaPenaltiesFixture.latePaymentPenaltyDetailModel

class LatePaymentPenaltyDetailSpec extends UnitSpec {
  ...
  "LatePaymentPenaltyDetail" should {

    "successfully read from valid json" in {
      downstreamJson.as[LatePaymentPenaltyDetail] shouldBe latePaymentPenaltyDetailModel
    }

    "fail to read from invalid json" in {
      JsObject.empty.validate[LatePaymentPenaltyDetail] shouldBe a[JsError]
    }

    "write to json" in {
      Json.toJson(latePaymentPenaltyDetailModel) shouldBe mtdJson
    }
