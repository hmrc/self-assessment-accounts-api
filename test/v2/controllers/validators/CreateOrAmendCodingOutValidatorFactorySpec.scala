...
class CreateOrAmendCodingOutValidatorFactorySpec extends UnitSpec with JsonErrorValidators with MockAppConfig {
  ...
  private val parsedBody = validJson.as[CreateOrAmendCodingOutRequestBody]
  ...
  "running a validation" should {
    "return the parsed domain object" when {
      "given a valid request" in {
        ...
        result shouldBe Right(
          CreateOrAmendCodingOutRequestData(parsedNino, parsedTaxYear, parsedBody)
        )
      }
      ...
      def testValidWithMissingField(field: String, taxCodeComponents: TaxCodeComponents): Unit = {
        ...
        withClue(s"Should be missing: $field") {
          ...
          result shouldBe Right(
            CreateOrAmendCodingOutRequestData(parsedNino, parsedTaxYear, expected)
          )
        }
      }
    ...
    "return RuleTaxYearNotEndedError" when {
      "given a tax year that hasn't ended" in {
        ...
        val result = validator(validNino, "2022-23", validJson).validateAndWrapResult()
        result shouldBe Left(
