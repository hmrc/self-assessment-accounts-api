  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in {
        validator.validate(RetrieveBalanceAndTransactionsRawData(validNino)) shouldBe Nil
      }
    }

    "return a path parameter format error" when {
      "an invalid nino is supplied" in {
        validator.validate(RetrieveBalanceAndTransactionsRawData("Nino")) shouldBe List(NinoFormatError)
      }
    }
  }
