...
class ListChargesValidatorSpec extends UnitSpec {

  val validator = new ListChargesValidator()

  "validation" should {
    "return no errors" when {
      "passed valid raw data" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-01"), Some("2019-02-02"))
        validator.validate(input) shouldBe List()
      }
    }
    "return a single error" when {
      "passed an invalid nino" in {
        val input = ListChargesRawRequest("AA123", Some("2019-02-01"), Some("2019-02-02"))
        validator.validate(input) shouldBe List(NinoFormatError)
      }
      "passed an invalid from date format" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-022-01"), Some("2019-02-02"))
        validator.validate(input) shouldBe List(FromDateFormatError)
      }
      "passed an invalid to date format" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-01"), Some("2019-022-02"))
        validator.validate(input) shouldBe List(ToDateFormatError)
      }
      "passed a missing from date" in {
        val input = ListChargesRawRequest("AA123456A", None, Some("2019-02-02"))
        validator.validate(input) shouldBe List(MissingFromDateError)
      }
      "passed a missing to date" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-01"), None)
        validator.validate(input) shouldBe List(MissingToDateError)
      }
      "passed a to date before the from date" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-02"), Some("2019-02-01"))
      ...
      "passed a to date which is the same as the from date" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-02"), Some("2019-02-02"))
        validator.validate(input) shouldBe List(RuleDateRangeInvalidError)
      }
      "passed a date range which is too large" in {
        val input = ListChargesRawRequest("AA123456A", Some("2019-02-02"), Some("2021-02-03"))
        validator.validate(input) shouldBe List(RuleDateRangeInvalidError)
      }
      "passed a from date which is too early" in {
        val input = ListChargesRawRequest("AA123456A", Some("2017-02-02"), Some("2018-02-03"))
        validator.validate(input) shouldBe List(RuleFromDateNotSupportedError)
      }
    }
    "return multiple errors" when {
      "multiple parameters are missing" in {
        val input = ListChargesRawRequest("AA123456A", None, None)
      ...
      "multiple invalid parameters are provided" in {
        val input = ListChargesRawRequest("AA123456A", Some("2017-02-02"), Some("2021-02-03"))
