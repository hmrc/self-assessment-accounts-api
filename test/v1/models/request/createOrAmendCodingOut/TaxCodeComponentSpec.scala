...
class TaxCodeComponentSpec extends UnitSpec {
  ...
  private val mtdJson = taxCodeComponentMtdJson

  private val desJson = taxCodeComponentDesJson

  private val requestBody = taxCodeComponent

  "TaxCodeComponent" when {
    "read from a valid Json" should {
      "produce the expected body" in {
        mtdJson.as[TaxCodeComponent] shouldBe requestBody
      }
    }
